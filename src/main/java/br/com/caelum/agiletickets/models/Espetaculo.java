...
public class Espetaculo {
	...
	public List<Sessao> criaSessoes(LocalDate inicio, LocalDate fim, LocalTime horario, Periodicidade periodicidade) {
		...
		if (periodicidade.equals(Periodicidade.DIARIA)) {
			LocalDate diaDeCriacaoDeSessao = new LocalDate();
			LocalTime horaDeCriacaoDeSessao = new LocalTime();
			int numeroDeDiasDeSessoes = Days.daysBetween(inicio, fim).getDays();
			int aceitarCriarSessao = Days.daysBetween(diaDeCriacaoDeSessao, inicio).getDays();
			
			
			if ( aceitarCriarSessao >= 0) {
				
			}

			// sessaoDiaria.setInicio(diaDeCriacaoDeSessao.toDateTime(horaDeCriacaoDeSessao));
			for (int i = 0; i < numeroDeDiasDeSessoes; i++) {
				Sessao sessaoDiaria = new Sessao();
				sessaoDiaria.setInicio(inicio.plusDays(i).toDateTime(horario));
				sessoes.add(sessaoDiaria);
			}
